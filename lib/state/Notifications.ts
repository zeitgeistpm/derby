...
export default class NotificationStore {
  items: SingleNotificationObservable[] = [];
  ...
  get broadcasting(): boolean {
    return this.items.findIndex(n => n.broadcast) !== -1;
  }
   ...
   */
  pushNotification(content: string, options?: {
    type?: NotificationType;
    lifetime?: number;
    autoRemove?: boolean;
    broadcast?: boolean;
    ...
    const numNotifications = this.items.length;

    if (numNotifications > 0) {
      const lastNotification = this.items[numNotifications - 1];
